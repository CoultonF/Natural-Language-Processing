...
class AttentionModule(nn.Module):
    ...
    def calcAlpha(self, decoder_hidden, encoder_out):
        ...
        # scores = []
        # for hi in encoder_out:
        #     score_i = self.W_enc(hi) + self.W_dec(decoder_hidden[0])
        #     scores.append(score_i)

        wencopt = self.W_enc(encoder_out.view(seq*batch, dim)).view(seq, batch, dim)
        wdecopt = self.W_dec(decoder_hidden[0]).unsqueeze(0).repeat([seq, 1, 1])
        scores = wencopt + wdecopt


        # scores = torch.stack(scores).squeeze(1).permute(1,0,2)
        # scores = torch.stack(scores)
        alpha = torch.nn.functional.softmax(self.V_att(torch.tanh(scores)), dim=0)
        return alpha
    ...
    def forward(self, decoder_hidden, encoder_out):
        ...
        # context = 0
        #
        # for ai,hi in zip(alpha,encoder_out):
        #     context += ai*hi

        context = torch.sum((alpha.repeat([1, 1, dim]) * encoder_out), dim=0)

        context = torch.sum(context, dim=0).reshape(1,1,dim)
...
def translate(model, test_iter):
    ...
    for i, batch in tqdm(enumerate(test_iter)):
        ...
        # source is the source sentence as a text string
        source = ' '.join(batch.dataset.examples[i].src)
        # unknowns is a object of tensors for the unk words in the output
        unknowns = ((output == 0).nonzero())
        # start a list for all unknown words for the current sentence.
        # the next few lines implement a list to store a simple copy mechanism that just takes the output word
        # representing the unknown word and replace it with the original source word.
        unknown_list = {}
        for i in range(unknowns.shape[0]):
            unk_ind = unknowns[i,0].item()
            unk = source.split(' ')[torch.max(attention.permute(0, 2, 1)[0, unk_ind], 0).indices.item()]
            unknown_list[unk_ind] = unk

        output = model.tgt2txt(output[:, 0].data).strip().split('<EOS>')[0]
        out_split = output.split()
        # replace output unk with german words from source
        for k, v in unknown_list.items():
            out_split[k] = v
        output = ' '.join(out_split)
