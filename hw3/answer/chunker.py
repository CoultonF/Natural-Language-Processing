...
class baseline_LSTMTaggerModel(nn.Module):

    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size):
        torch.manual_seed(1)
        super(baseline_LSTMTaggerModel, self).__init__()
    ...
    def forward(self, sentence, tensor_char):
        embeds = self.word_embeddings(sentence)
        new_input = torch.cat((embeds,tensor_char), dim = 1)
        # lstm_out, _ = self.lstm(embeds.view(len(sentence), 1, -1))
        lstm_out, _ = self.lstm(new_input.view(len(sentence), 1, -1))
        tag_space = self.hidden2tag(lstm_out.view(len(sentence), -1))
        tag_scores = F.log_softmax(tag_space, dim=1)
        return tag_scores

class baseline_LSTMTagger:
    ...
    def __init__(self, trainfile, modelfile, modelsuffix, unk="[UNK]", epochs=10, embedding_dim=128, hidden_dim=64):
        ...
        self.model = baseline_LSTMTaggerModel(self.embedding_dim, self.hidden_dim, len(self.word_to_ix), len(self.tag_to_ix))
    ...
    def argmax(self, seq):
        ...
        char_level = []
        with torch.no_grad():
            char_level = [char_rep(word) for word in seq]
            tensor_char = torch.FloatTensor(char_level)
            inputs = prepare_sequence(seq, self.word_to_ix, self.unk)
            tag_scores = self.model(inputs, tensor_char)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            for sentence, tags in tqdm.tqdm(self.training_data):
                ...
                char_level = [char_rep(word) for word in sentence]
                ...
                tag_scores = self.model(sentence_in,torch.FloatTensor(char_level))
...
class LSTMTaggerModel(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size):
        ...
        self.lstm2 = nn.LSTM(300, hidden_dim, bidirectional=False)
    ...
    def forward(self, sentence, tensor_char):
        embeds = self.word_embeddings(sentence)
        in2, _ = self.lstm2(tensor_char.view(len(sentence), 1, -1))
        new_input = torch.cat((embeds,in2.view(len(sentence), -1)), dim = 1)
        # lstm_out, _ = self.lstm(embeds.view(len(sentence), 1, -1))
        lstm_out, _ = self.lstm(new_input.view(len(sentence), 1, -1))
        tag_space = self.hidden2tag(lstm_out.view(len(sentence), -1))
        tag_scores = F.log_softmax(tag_space, dim=1)
...
class LSTMTagger:
    ...
    def argmax(self, seq):
        ...
        with torch.no_grad():
            char_level = [char_rep(word) for word in seq]
            tensor_char = torch.FloatTensor(char_level)
            inputs = prepare_sequence(seq, self.word_to_ix, self.unk).to('cpu')
            tag_scores = self.model(inputs, tensor_char)
            for i in range(len(inputs)):
                output.append(self.ix_to_tag[int(tag_scores[i].argmax(dim=0))])
        return output
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for sentence, tags in tqdm.tqdm(self.training_data):
                ...
                # Step 3. Run our forward pass.
                tag_scores = self.model(sentence_in,torch.FloatTensor(char_level).to(device))
    ...
    def decode(self, inputfile):
        ...
        saved_model = torch.load(self.modelfile + self.modelsuffix, map_location='cpu')
        self.model.load_state_dict(saved_model['model_state_dict'])
        self.optimizer.load_state_dict(saved_model['optimizer_state_dict'])
        epoch = saved_model['epoch']
        loss = saved_model['loss']
...
if __name__ == '__main__':
    ...
    optparser.add_option("-b", "--baselinemodelfile", dest="baseline_modelfile", default=os.path.join('data', 'baseline'), help="filename without suffix for baseline model files")
    ...
    # baseline_modelfile = opts.baseline_modelfile
    # if opts.baseline_modelfile[-4:] == '.tar':
    #     baseline_modelfile = opts.baseline_modelfile[:-4]
    # baseline_chunker = baseline_LSTMTagger(opts.trainfile, baseline_modelfile, opts.modelsuffix, opts.unk)
    # # use the model file if available and opts.force is False
    # if os.path.isfile(opts.baseline_modelfile + opts.modelsuffix) and not opts.force:
    #     decoder_output = baseline_chunker.decode(opts.inputfile)
    # else:
    #     print("Warning: could not find modelfile {}. Starting training.".format(baseline_modelfile + opts.modelsuffix), file=sys.stderr)
    #     baseline_chunker.train()
    #     decoder_output = baseline_chunker.decode(opts.inputfile)


    # For running the improved version:
